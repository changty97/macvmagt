...
func init() {
	...
	// mTLS flags for agent
	rootCmd.PersistentFlags().StringVar(&cfg.CACertPath, "ca-cert-path", cfg.CACertPath, "Path to CA certificate (for mTLS)")
	rootCmd.PersistentFlags().StringVar(&cfg.ServerCertPath, "server-cert-path", cfg.ServerCertPath, "Path to server certificate (for agent's listener)")
	rootCmd.PersistentFlags().StringVar(&cfg.ServerKeyPath, "server-key-path", cfg.ServerKeyPath, "Path to server private key (for agent's listener)")
	rootCmd.PersistentFlags().StringVar(&cfg.ClientCertPath, "client-cert-path", cfg.ClientCertPath, "Path to client certificate (for agent sending heartbeats)")
	rootCmd.PersistentFlags().StringVar(&cfg.ClientKeyPath, "client-key-path", cfg.ClientKeyPath, "Path to client private key (for agent sending heartbeats)")
